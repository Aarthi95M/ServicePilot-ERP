// app/(tabs)/attendance.tsx — Check in/out with GPS + history
...
export default function AttendanceScreen() {
  ...
  const checkIn  = useMutation({ mutationFn: attendanceApi.checkIn,  onSuccess: () => qc.invalidateQueries({ queryKey: ['attendance'] }) });
  const checkOut = useMutation({ mutationFn: attendanceApi.checkOut, onSuccess: () => qc.invalidateQueries({ queryKey: ['attendance'] }) });
    ...
    fn.mutate(
      { latitude: coords.latitude, longitude: coords.longitude },
      {
        onSuccess: (res) => {
          ...
          else Alert.alert('✅', type === 'in' ? 'Checked in successfully!' : 'Checked out successfully!');
        },
  ...
  return (
    <ScrollView
      ...
      refreshControl={<RefreshControl refreshing={false} onRefresh={() => { qc.invalidateQueries({ queryKey: ['attendance-today'] }); refetch(); }} tintColor={Colors.primary} />}
    >
      {/* Today's card */}
      <Card style={[styles.todayCard, checkedIn && styles.todayCardIn]}>
        ...
        {loadingToday ? <LoadingSpinner size="small" /> : (
          <>
            {/* Status circle */}
            <View style={[styles.statusCircle, { backgroundColor: checkedIn ? '#dcfce7' : checkedOut ? '#f1f5f9' : '#fef9c3' }]}>
            ...
            <Text style={styles.statusText}>
              {checkedIn ? 'You are checked in' : checkedOut ? 'Checked out for today' : 'Not checked in'}
            </Text>
            ...
            {today?.checkInTime && (
              <View style={styles.timesRow}>
                ...
                {today.totalHours != null && <TimeChip label="Hours" time={`${today.totalHours?.toFixed(1)}h`} raw />}
              </View>
            )}
      ...
      {loadingHistory ? <LoadingSpinner /> : records.length === 0 ? (
        <EmptyState title="No records yet" subtitle="Your attendance history will appear here." icon="📅" />
      ) : (
        records.map((rec: any) => (
          <Card key={rec.id} style={styles.histRow}>
            <View style={styles.histLeft}>
              <Text style={styles.histDate}>{formatDate(rec.attendanceDate ?? rec.checkInTime)}</Text>
              <View style={styles.histTimes}>
                <Text style={styles.histTime}>In: {formatTime(rec.checkInTime)}</Text>
                {rec.checkOutTime && <Text style={styles.histTime}>Out: {formatTime(rec.checkOutTime)}</Text>}
              </View>
            </View>
            <View style={styles.histRight}>
              {rec.totalHours != null && <Text style={styles.histHours}>{rec.totalHours?.toFixed(1)}h</Text>}
